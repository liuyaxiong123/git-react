...
Snake.prototype.init=function (){
    ...
    snakeHead.create();
    this.head=snakeHead;//存储蛇头的信息存起来
    this.pos.push([2,0]);;//把蛇头的位置存起来

    // 创建蛇身体1
    var snakeBody1=new Square(1,0,'snakeBody');
    snakeBody1.create();
    this.pos.push([1,0]);;//把蛇什1的坐标存起来

    var snakeBody2=new Square(0,0,'snakeBody');
    snakeBody2.create();
    this.tail=snakeBody2;//把蛇尾的的信息存起来
    this.pos.push([0,0]);;//把蛇什1的坐标存起来
...
Snake.prototype.getNextPos=function (){
    ...
    this.pos.forEach(function (value){
        if(value[0]==nextPos[0] && value[1]==nextPos[1]){
            //如果数组中的两个数字都相等，就说明下一个点在蛇身上能找到，代表撞到自己
            selfCollied=true;
        }
    })
    if(selfCollied){
...
Snake.prototype.strategies={
    move:function(format){ //这个参数用来决定用不用删除最后一个方块
        // 创建一个新的身体(在旧蛇头的位置)
...
function createFood(){
    ...
    var include=true;//循环跳出的条件，true表示食物的坐标在蛇身上((继续循环)，false表示食物的坐标不在蛇身上(不循环)；
    while(include){
...
Game.prototype.init=function (){
    ...
    document.onkeydown=function(ev){
        if(ev.keyCode==37 && snake.direction!=snake.directionNum.right){//用户按下左键的时候这条蛇不能是正在往右走
